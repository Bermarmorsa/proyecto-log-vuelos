...
def estacion_mas_cercana(lat, lon, estaciones):
    ...
    logger.info("Buscando estacion mas cercana")
    objetivo = (lat, lon)
    ...
    for est in estaciones:
        ...
        if d < dist_min:
            dist_min = d
            logger.info(f"dist_min = {dist_min}")
...
def consultar_meteo(lat, lon, fecha_inicio, fecha_fin):
    ...
    for periodo in periodos:
        ...
        fecha_inicio_pre = periodo[0]
        fecha_fin_per = periodo[1]

        datosnew = datos_estacion(est["idema"], fecha_inicio_pre, fecha_fin_per)

        datos = pd.concat([datos, datosnew], ignore_index=True)
        logger.info(f"Generado dataframe de periodo {periodo[0]} - {periodo[1]}")

    ruta_meteo = BASE_DIR / "archivos_salida"
    nombre_archivo= f'meteo_{fecha_inicio[0:10]}_{fecha_fin[0:10]}.csv'
    ruta_csv_meteo = Path(ruta_meteo / nombre_archivo)


    datos.to_csv( ruta_csv_meteo, index=False)
    logger.info(f"Generado el csv en ruta {ruta_csv_meteo}")
